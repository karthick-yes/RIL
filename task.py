...
class Task:
    ...
    def __init__(self, name: str, ID: int, priority: int, deadline: str, duration: float,
                 dependencies: List[int], source_file: str, status: float = 0.0, time_spent: float = 0.0):
        self.name = name
        self.ID = ID
        ...
        if isinstance(deadline, str): # Handle cases where deadline might already be datetime
            try:
                self.deadline = datetime.fromisoformat(deadline)
            except ValueError:
                print(f"Warning: Invalid deadline format '{deadline}' for task '{name}'. Using default past date.")
                self.deadline = datetime.fromisoformat('1970-01-01T00:00:00') # Default past date
        elif isinstance(deadline, datetime):
             self.deadline = deadline
        else:
             print(f"Warning: Unexpected deadline type {type(deadline)} for task '{name}'. Using default past date.")
        ...
        self.status = max(0.0, min(self.status, 1.0)) # Clamp status between 0.0 and 1.0
        ...
        if self.ID in Task.registry:
            ...
            print(f"Warning: Task with ID {self.ID} already exists in registry. Overwriting.")
    ...
    def update_from_dict(self, task_dict: Dict, source_file: str) -> None:
        ...
        if 'priority' in task_dict:
            try:
                self.priority = int(task_dict['priority'])
                 ...
                 print(f"Warning: Invalid priority format '{task_dict.get('priority')}' for task '{self.name}'. Keeping old value: {self.priority}")
        ...
        if 'deadline' in task_dict:
            ...
            if isinstance(deadline_val, str):
                try:
                    ...
                    print(f"Warning: Invalid deadline format '{deadline_val}' for task '{self.name}'. Keeping old value: {self.deadline}")
            elif isinstance(deadline_val, datetime):
                self.deadline = deadline_val
            else:
                 print(f"Warning: Unexpected deadline type {type(deadline_val)} for task '{self.name}'. Keeping old value: {self.deadline}")
        ...
        if 'duration' in task_dict:
            try:
                ...
                print(f"Warning: Invalid duration format '{task_dict.get('duration')}' for task '{self.name}'. Keeping old value: {self.duration}")
        ...
        if 'status' in task_dict:
            try:
                ...
                print(f"Warning: Invalid status format '{task_dict.get('status')}' for task '{self.name}'.")
        ...
        if 'time_spent' in task_dict:
            try:
                ...
                print(f"Warning: Invalid time_spent format '{task_dict.get('time_spent')}' for task '{self.name}'.")
        ...
        if not update_status_explicitly and ('duration' in task_dict or 'time_spent' in task_dict):
             if self.duration > 0:
                 self.status = self.time_spent / self.duration
             else:
                 self.status = 1.0 if self.time_spent > 0 else 0.0 # Handle duration 0
    ...
    def computePriorityScore(
        ...
        days_to_deadline = time_until_deadline_seconds / (24 * 3600.0) # Convert to days as float

        # Ensure max_horizon is positive for division (guaranteed by environment.py now, but defensive)
        if max_horizon <= 0:
             deadline_urgency = 1.0 # Maximum urgency if horizon is invalid or zero scale
        else:
        ...
        base_priority = min(self.priority / max_priority, 1.0) if max_priority > 0 else 0.0
        ...
        for dep_id in self.dependencies:
            ...
            if dep_task:
                ...
                dep_priorities.append(min(dep_task.priority / max_priority, 1.0) if max_priority > 0 else 0.0)

        # Average normalized priority of dependencies
        avg_norm_priority_deps = sum(dep_priorities) / len(dep_priorities) if dep_priorities else 0.0
        ...
        dependency_factor = max(0.0, min(dependency_factor, 1.0)) # Clamp combined factor


        # --- Completion Status ---
        # Score higher for tasks that are less complete (more work remaining)
        completion_status_factor = 1.0 - max(0.0, min(self.status, 1.0)) # Ensure self.status is clamped 0-1
        ...
        normalized_duration = min(self.duration / max_duration, 1.0) if max_duration > 0 else 0.0
        ...
        for dep_id in self.dependencies:
            ...
            if subtask:
                 ...
                 sub_time_until_deadline_seconds = (subtask.deadline - current_datetime).total_seconds()
                 ...
                 sub_base_prio = min(subtask.priority / max_priority, 1.0) if max_priority > 0 else 0.0 # Normalize subtask priority
                 subtask_scores.append((sub_urgency + sub_base_prio) / 2.0) # Simple average of two factors for dependency's influence


        subtask_influence = sum(subtask_scores) / len(subtask_scores) if subtask_scores else 0.0
        ...
        score = (self.w1 * deadline_urgency +
    ...
    def update_task_status(cls, task_id: int, status: float) -> bool:
        ...
        if task:
            ...
            print(f"Task '{task.name}' (ID: {task_id}) status updated to {task.status:.2f}")
            return True
        else:
    ...
    def update_time_spent(cls, task_id: int, time_spent: float) -> bool:
        ...
        if task:
            ...
            print(f"Task '{task.name}' (ID: {task_id}) time spent updated to {task.time_spent:.2f}, status recalculated to {task.status:.2f}")
            return True
        else:
